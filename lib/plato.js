...
exports.getOverviewReport = function (reports) {
  ...
  var messagesObj,
  populateMessages=function(type,code){
     messagesObj[type][code]=messagesObj[type].hasOwnProperty(code)?messagesObj[type][code]+1:1;
      //messagesObj[type]=messagesObj.hasOwnProperty(type)?(messagesObj[type]*1)+1:1;
  };
  ...
  reports.forEach(function(report){
    ...
    summary.total.functions+=report.complexity.functions.length;
    ...
    report.jshint.messages.forEach(function(message){
      switch(message.errorcode[0]){
        case "E":
          populateMessages('errors',message.errorcode);
          break;
        case "W":
          populateMessages('warnings',message.errorcode);
          break;
        case "I":
          populateMessages('info',message.errorcode);
          break;
        default:
          populateMessages('info',message.errorcode);
          break;
    ...
    culledReports.push({
      ...
      jshint : {
        ...
        messageClassification:messagesObj
      },
      complexity : {
        aggregate : aggregate,
  ...
  summary.total.files = reports.length;
  summary.average.functions = ~~(summary.total.functions/reports.length);
