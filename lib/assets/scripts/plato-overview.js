...
//      value = el.data('errors');
//      el.append(horizontalBar(value, Math.min(value * 5, width), 'messageTypes errors', [1,10]));
  ...
  function drawOverviewCharts(reports) {
    ...
    reports.forEach(function(report){
      ...
      var newData=report;
      for (var type in newData.jshint.messageClassification) {
        if(newData.jshint.messageClassification.hasOwnProperty(type)){
          for (var messageType in newData.jshint.messageClassification[type]) {
            messageTypes.data.push({
              value : newData.jshint.messageClassification[type][messageType],
              label : messageType,
              file  : report.info.fileShort
            }); 
          };
                  console.log(type);
        }
    ...
    if (__options.flags.jshint){
     charts.push(Morris.Bar(lint));
     charts.push(Morris.Bar(messageTypes));
